...
- ✅ **Quotation — round 2, part 4: drag-to-reorder lines** — SortableJS per group `<tbody>` (drag handle ⠿, only when the doc is editable); `onEnd` → `htmx.ajax` POST to `quotation_reorder_lines` with the group's line PKs in their new order → reassigns just the `position` slots those rows occupy (so dragging within one room/group leaves the rest of the document alone) → returns the `_quote_lines.html` partial. Init via a nonce'd inline script + `htmx.onLoad` so it re-arms after htmx swaps. Reorder is refused (403) once the doc is locked. `make check` green (120 tests).

## Phase 1 backlog (next, suggested order — see REQUIREMENTS.md §4)

1. **Quotation — round 2, remaining** — per-line image upload + show it in the PDF (needs WeasyPrint `base_url`/media — see CLAUDE.md §7); cross-group line drag (currently within-group only); LINE send (Messaging API); approval audit trail (who/when/note as rows, not just the latest `approved_by`); full revision diff (compare any two revisions field-by-field). §4.7
