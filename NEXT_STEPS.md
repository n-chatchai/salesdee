...
- ✅ **Quotation — round 2, part 3: INCLUSIVE-VAT pricing + rounding line** — `compute_document_totals` now handles `price_mode = INCLUSIVE`: line amounts are treated as VAT-inclusive, the 7% base is backed out (`amount / 1.07`), the grand total = gross-after-discount; withholding is computed on the ex-VAT base in both modes. `DocumentTotals` gained `inclusive` + `rounding` (reconciliation residual, shown as a "ปัดเศษ" line in the editor / public page / PDF when non-zero — always 0 in exclusive mode). `price_mode` is now a field on `QuotationForm` (optional, defaults to exclusive). Exclusive-mode results are unchanged. `make check` green (118 tests).

## Phase 1 backlog (next, suggested order — see REQUIREMENTS.md §4)

1. **Quotation — round 2, remaining** — per-line image upload + show it in the PDF; line reordering (drag); LINE send (Messaging API); approval audit trail (who/when/note as rows, not just the latest `approved_by`); full revision diff (compare any two revisions field-by-field). §4.7
