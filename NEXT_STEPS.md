...
- ✅ **Quotation htmx line editor** — `_quote_lines.html` partial (room-grouped tables + totals box + add-line form, root `#quote-lines`); add / inline-edit / delete a line via htmx (returns the partial → totals update live); picking a catalog product/variant auto-fills tax/unit (authoritative) + price/desc/dims/material (if blank) via `apply_catalog_defaults`. `make check` green (97 tests).

## Phase 1 backlog (next, suggested order — see REQUIREMENTS.md §4)

1. **Quotation — round 2, remaining** — document statuses + transitions (submit-for-approval / approve / cancel / auto-expire past `valid_until`); **revisions** (snapshot on send + diff between revisions); **discount-approval workflow** (`Membership.max_discount_percent`/`approval_limit` — require approval over the cap); INCLUSIVE-VAT pricing + rounding-diff line; per-line image upload + show it in the PDF; line reordering (drag); LINE send (Messaging API). §4.7
