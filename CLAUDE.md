...
- **UI must match the design.** Visual sources of truth (in priority order):
  1. **`design/brand-guide.html` — supreme authority.** Canonical for tokens (palette, type scale, spacing, radius), the `.ic` letter-mark icon system, logo lockup, and component primitives. **When any other deck disagrees with the brand guide, the brand guide wins.** Some logos / icons / spacing inside the screen decks are off-spec — do not copy them; use the brand-guide version.
  2. **`design/backoffice.html`** (27 frames) — back-office screens (auth, onboarding, home queue, quote, customer, catalog, settings, mobile, states).
  3. **`design/tenant-site.html`** (12 frames) — public per-tenant buyer site (landing, browse, product, cart, request form, post-submit, LINE webview, search, mobile).
  4. **`design/website.html`** — salesdee.com marketing site.
  5. **`design/prd.md`** — product decisions (PRD v2.2).

  When you build or change a screen, open the matching deck frame (`.frame` after the `.frame-label` whose `.id` names it) and reproduce its layout, spacing, and components — don't ship a "functional but rough" approximation — but cross-check every token / icon / logo against the brand guide and override the deck where they differ. Reuse the design tokens in `static/css/tokens.css` (`var(--accent) --ai --paper --ink --radius --shadow-*` …) and the shared classes in `static/css/app.css` (`.btn .chip .card .kpi .ai-panel .tbl .field …`); put screen-specific CSS in a per-screen file under `static/css/` (`dashboard.css`, `kanban.css`, `quote-editor.css`, …) linked via `{% block extra_css %}`. Never hardcode colors — use the tokens. AI surfaces are always sage (`var(--ai)` family / `.ai-panel` / `.ai-suggest`), never the persimmon accent. Keep `static/css/mobile.css` degrading any new multi-column layout. PDF/email templates are the exception — they're standalone, not the app shell.
